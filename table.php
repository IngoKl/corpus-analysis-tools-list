...
function genTagListFrequency($sheet_data)
{
	...
	foreach ($sheet_data as $row) {
		...
		foreach ($categories as $category) {
			if (isset($tag_list[trim($category)])) {
				$tag_list[trim($category)]++;
			} else {
				$tag_list[trim($category)] = 1;
			}
		}
	}
...
function printLastChangeDate()
{
	...
	$change_time = filemtime($config_sheet_file);
...
function saveLocalData($sheet_data)
{
	...
	$local_file = fopen($config_sheet_file, 'w');
 ...
 */
function loadLocalData($max_age = 9999999)
{
	include('config.php');

	if (time() - filemtime($config_sheet_file) < $max_age) {
		$local_file = file_get_contents($config_sheet_file);
...
if (loadLocalData($config_max_age)) {
	...
	if (($handle = fopen($config_sheet_url, 'r')) !== False) {
		$sheet_data_local = loadLocalData();

		while (($data = fgetcsv($handle, 1000, ',')) !== False) {
			$sheet_data_remote[] = $data;
		}
		fclose($handle);
